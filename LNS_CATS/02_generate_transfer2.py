...
class SamplingAgent(scip.Branchrule):
    ...
    def branchexeclp(self, allowaddcons):
        ...
        if self.model.getNNodes() == 1:
            ...
            state_khalil = utilities.extract_khalil_variable_features(self.model, [], self.khalil_root_buffer)
...
#            K = [x.getCol() for x in self.model.getVars()]  
#            print(K)

#             COL = self.model.getLPColsData()
#             print([i.getVar() for i in COL])
...
#         print(K)
#         print(m.getSolVal(L,'t_x1'))
   

        filename = '{}/sample_{}_{}.pkl'.format(out_dir,episode,1)
        with gzip.open(filename, 'wb') as f:
            pickle.dump({
                'episode': episode,
                'instance': instance,
                'seed': seed,
                'node_number': m.getNNodes(),
                'obj': m.getObjVal(),
                'status': m.getStatus(),                
                'label': K,
        }, f)
        ...
        print("[w {}] episode {} done, {} samples".format(os.getpid(),episode,branchrule.sample_counter))
            ...
            for sample in samples_to_write:
                ...
                # else write sample
                    ...
                    if sample['type'] == 'sample':                                                
                        os.rename(sample['filename'], '{}/sample_{}.pkl'.format(out_dir,sample['episode']+1))
                        
#                         in_buffer -= 1
#                         i += 1
#                         print("[m {}] {} / {} samples written, ep {} ({} in buffer).".format(os.getpid(),i,n_samples,sample['episode'],in_buffer))

#                         # early stop dispatcher (hard)
#                         if in_buffer + i >= n_samples and dispatcher.is_alive():
#                             dispatcher.terminate()
#                             print("[m {}] dispatcher stopped...".format(os.getpid()))
...
#                             break
                        ...
                        os.rename(sample['filename'], '{}/sampleR_{}.pkl'.format(out_dir,sample['episode']+1))
...
if __name__ == '__main__':
    ...
    if args.problem == 'setcover':
        instances_transferM = glob.glob('data/instances/setcover/transfer_1000r_1000c_0.05d/*.lp')
        instances_transferL = glob.glob('data/instances/setcover/transfer_2000r_1000c_0.05d/*.lp')
    ...
    print("{} transferM instances for {} samples".format(len(instances_transferM),transfer_size))
    print("{} transferL instances for {} samples".format(len(instances_transferL),transfer_size))

    # create output directory, throws an error if it already exists
#    os.makedirs(out_dir)

    rng = np.random.RandomState(args.seed+3)
    collect_samples(instances_transferM, out_dir + '/transferM', rng, transfer_size,
    ...
    collect_samples(instances_transferL, out_dir + '/transferL', rng, transfer_size,
                    args.njobs, exploration_policy=exploration_strategy,
                    query_expert_prob=node_record_prob,
                    time_limit=time_limit)
