...
def learn(args,network='mlp',
          ...
          render=False,
          render_eval=False,
          noise_type=None, #'adaptive-param_0.01',
          ...
          gamma=0.99,  #0.9 #0.96
          clip_norm=None,
          nb_train_steps=4, # per epoch cycle and MPI worker,  50 10 30   100  10  3 1
          nb_eval_steps=100, #20 #50
          batch_size=250, # per MPI worker  64 32  64   128   64  128 300
          tau=0.01,
          eval_env=None,
          load_path=None,
    ...
    if args.problem == 'cauctions':
        ...
        instances_valid += ["data/instances/cauctions/valid_2000_4000/instance_{}.lp".format(i+1) for i in range(20)]
    ...
    memory = Memory(limit=int(1e3), action_shape=(4000,1,), observation_shape=(4000,22,))
...
#                obs=init_sols
                ...
                pre_sols = np.ones([2,batch_sample,4000])
...
#                    action[cur_sols[:,:,np.newaxis]==next_sols[:,:,np.newaxis]]=0.
                    ...
                    if t_rollout>0:
                        agent.store_transition(cur_obs_s, action_s, r_s, next_obs_s, action)
...
#                            next_obs = np.concatenate((formu_feat, inc_val[:,:,np.newaxis], avg_inc_val[:,:,np.newaxis], cur_sols[:,:,np.newaxis]), axis=-1) 
                ...
                if save_path is not None and ave>max_obj:
