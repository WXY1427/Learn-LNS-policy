...
from ddpg.models import Actor_mean, Critic_mean, Actor_mean_semi
...
def collect_samples0(instances, out_dir, rng, n_samples, n_jobs,
    ...
    for sample in out_Q:
        ...
        print(np.transpose(sample['state'][1]['incidence']).shape)
...
def learn(args,network='mlp',
          ...
          batch_size=80, # per MPI worker  64 32  64   128   64  128 300
          tau=0.01,
          eval_env=None,
          load_path=None,
    ...
    if args.problem == 'maxcut':
        ...
        out_dir = 'data/samples/maxcut/500r_100c_0.05d'
    else:
        raise NotImplementedError
    ...
    memory = Memory(limit=int(490), action_shape=(2975,1,), observation_shape=(2975,15,))
    ...
    for epoch in range(nb_epochs): 
        ...
        with open("ddpg_mean_results/{}".format(result_file), 'w', newline='') as csvfile:
            ...
            for cycle in range(nb_epoch_cycles):        # nb_epoch_cycles
                ...
                if args.problem == 'maxcut':     
                    init_sols = ini_sol          
#                    init_sols = np.stack([np.zeros(2975) for _ in range(batch_sample)])                      
#                obs=init_sols

                ori_objs=np.copy(best_root)  
#                ori_objs=np.zeros(batch_sample) 
                ...
                rec_inc_obj = [[] for r in range(batch_sample)]  #ADD

                [rec_inc[r].append(init_sols[r]) for r in range(batch_sample)]  #ADD 

                [rec_inc_obj[r].append(best_root[r]) for r in range(batch_sample)]  #ADD 
                ...
                pre_sols = np.ones([2,batch_sample,2975])
...
#                cur_obs = np.concatenate((formu_feat, inc_val[:,:,np.newaxis], avg_inc_val[:,:,np.newaxis], cur_sols[:,:,np.newaxis]), axis=-1)      
                mask = None    

                nat_num = np.arange(nb_actions)     
...
#                            next_obs = np.concatenate((formu_feat, inc_val[:,:,np.newaxis], avg_inc_val[:,:,np.newaxis], cur_sols[:,:,np.newaxis]), axis=-1) 
...
#                            print(time.time()-sep_t) 
                ...
                if save_path is not None and ave>max_obj and ave<1000000:
