...
class DDPG(object):
    def __init__(self, actor, critic, memory, observation_shape, action_shape, param_noise=None, action_noise=None,
        ...
        self.obs0 = tf.placeholder(tf.float32, shape=(None,) + observation_shape + (4965,), name='obs0')
        self.obs1 = tf.placeholder(tf.float32, shape=(None,) + observation_shape + (4965,), name='obs1')
...
#        self.actor_loss = -tf.reduce_mean(tf.squeeze(tf.reduce_sum(self.inter,1))*self.Q0)                      #QA2C
    ...
    def train(self, IM):
        ...
        if self.normalize_returns and self.enable_popart:
            ...
            act_loss = self.sess.run(self.actor_loss, feed_dict={
            self.obs0: np.concatenate((batch['obs0'], IM_batch), axis=-1),
            self.actions: batch['actions'],
