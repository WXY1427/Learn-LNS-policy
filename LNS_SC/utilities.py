...
def log(str, logfile=None):
    str = '[{}] {}'.format(datetime.datetime.now(),str)
    print(str)
...
def init_scip_paramsHH(model, seed, heuristics=False, presolving=False, separating=False, conflict=False):
...
def init_scip_paramsR(model, seed, heuristics=True, presolving=True, separating=True, conflict=True):
...
#    col_feat_names = [[k, ] if v.shape[1] == 1 else ['{}_{}'.format(k,i) for i in range(v.shape[1])] for k, v in col_feats.items()]
#    col_feat_names = [n for names in col_feat_names for n in names]

#    col_feat_vals = np.concatenate(list(col_feats.values()), axis=-1)

#    col_feat_names = ['reduced_cost','has_lb','coef_normalized','avg_inc_val','age','sol_is_at_ub','sol_is_at_lb','sol_frac','type','has_ub','inc_val','basis_status','sol_val']

    col_feat_names = ['coef_normalized','reduced_cost','has_lb','age','sol_is_at_ub','sol_is_at_lb','sol_frac','has_ub','basis_status','sol_val']

#    col_feat_names = ['reduced_cost','has_lb','coef_normalized','age','sol_is_at_ub','sol_is_at_lb','sol_frac','type','has_ub','basis_status','sol_val']

    col_feat_vals = np.concatenate([col_feats[i] for i in col_feat_names], axis=-1)
...
#    row_feat_names = [[k, ] if v.shape[1] == 1 else ['{}_{}'.format(k,i) for i in range(v.shape[1])] for k, v in row_feats.items()]
...
#    row_feat_vals = np.concatenate(list(row_feats.values()), axis=-1)
    ...
    row_feat_vals = np.concatenate([row_feats[i] for i in row_feat_names], axis=-1)
